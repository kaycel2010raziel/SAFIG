$(document).ready(function(){
	...
	$("#Crear_Tabla").html("<div class='container-fluid'><div class='table-responsive h-100 ' id='listadoregistros2'><table id='tbllistado2' class='table table-hover table-bordered text-center'><thead class='thead-dark'><th>NO.</th><th>NOMBRE</th><th>CORREO</th><th>PASSWORD</th><th>ROL</th><th>INSTITUCION</th><th>ESTADO</th><th>ACCIONES</th></thead><tbody id='tbllistado_body2'></tbody></table></div></div>");
	$("#new_user").click(function(){New_User();});	
	$("#PDF").click(function(){PDF_USER();});	
	
	//TABLA DE USUARIOS
	$.ajax({type: "POST", url: "../php/TOWER_ADMIN_USER.php", async: true, data: {a0: 1}})
	.done(function(stream) {
		data = jQuery.parseJSON(stream);
		$("#listadoregistros2 tbody").empty();
		console.log(data);
		data.forEach(function(element, index, array){
			switch(element['FK_ESTADO']){
				case '1':
					var EstadoClass1 = 'alert-success';
					break;
				case '2':
					var EstadoClass1 = 'alert-danger';
					break;
			}
			...
			//$("#listadoregistros2_tr_"+index).append("<td>"+element['PASSWORD']+"</td>");
			$("#listadoregistros2_tr_"+index).append("<td>*******</td>");
			$("#listadoregistros2_tr_"+index).append("<td>"+element['ROL']+"</td>");
			$("#listadoregistros2_tr_"+index).append("<td>"+element['INSTITUCION']+"</td>");
		...
		$("#tbllistado2").DataTable({
			...
			fnDrawCallback : function() {
				...
				$('.gen_auth_code').off().click(function(){					
					var id_usuario = $(this).attr("id_usuario");
					var respuesta = confirm("Estas seguro?, el Token actual dejara de funcionar");
					if(respuesta == false){
						location.reload();
					} else {
						$.ajax({type: "POST", url: "../php/TOWER_ADMIN_USER.php", async: true, data: {a0: 11, id_usuario: id_usuario}})
						.done(function(stream) {
							window.open('../print/AUTH_TOKEN.php?URL='+stream);
						});
					}					
				});
			}
	...
	function New_User(){
		...
		//CARGAR ROLES//
		$.ajax({type: "POST", url: "../php/TOWER_ADMIN_USER.php", async: true , data: {'a0': 3}})
		.done(function(stream) {	
			data = jQuery.parseJSON(stream);
			$("#Select_Rol").append('<option  value="0" selected="selected">Seleccione Rol</option>');
			data.forEach(function(element, index, array){
				$("#Select_Rol").append('<option  value="'+element['ID_ROL']+'">'+element['NOMBRE']+'</option>');
			});
		}); 
